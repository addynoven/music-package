...
__export(index_exports, {
  ...
  EssentiaAnalysisProvider: () => EssentiaAnalysisProvider,
  HttpError: () => HttpError,
...
var SongSchema = import_zod.z.object({
  ...
  videoId: import_zod.z.string().min(1).refine(
...
var CamelotSchema = import_zod2.z.enum(CAMELOT_VALUES);
var TempoSchema = import_zod2.z.object({
  bpm: import_zod2.z.number().positive(),
  confidence: import_zod2.z.number().min(0).max(1),
  beatGrid: import_zod2.z.array(import_zod2.z.number().nonnegative())
});
var OnsetsSchema = import_zod2.z.array(import_zod2.z.number());
var KeySchema = import_zod2.z.object({
  tonic: import_zod2.z.enum(["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"]),
  mode: import_zod2.z.enum(["major", "minor"]),
  camelot: CamelotSchema,
  confidence: import_zod2.z.number().min(0).max(1)
});
var EnergyPointSchema = import_zod2.z.object({
  t: import_zod2.z.number(),
  rms: import_zod2.z.number()
});
var EnergySchema = import_zod2.z.object({
  overall: import_zod2.z.number(),
  envelope: import_zod2.z.array(EnergyPointSchema).optional()
});
var AnalysisSectionSchema = import_zod2.z.object({
  start: import_zod2.z.number(),
  end: import_zod2.z.number(),
  label: import_zod2.z.string(),
  loudness: import_zod2.z.number()
});
var AnalysisSchema = import_zod2.z.object({
  videoId: import_zod2.z.string().min(1),
  duration: import_zod2.z.number().positive(),
  tempo: TempoSchema,
  onsets: OnsetsSchema,
  key: KeySchema.nullable(),
  energy: EnergySchema.nullable(),
  sections: import_zod2.z.array(AnalysisSectionSchema).nullable(),
  analyzedAt: import_zod2.z.string().min(1)
});
...
async function fetchPCM(videoId, cookiesPath) {
  const cookiesArgs = cookiesPath ? ["--cookies", cookiesPath] : [];
  const ytdlp = (0, import_node_child_process4.spawn)("yt-dlp", [
    "--no-playlist",
    ...cookiesArgs,
    "-f",
    "bestaudio",
  ...
  const ffmpeg = (0, import_node_child_process4.spawn)("ffmpeg", [
    "-hide_banner",
    "-loglevel",
    "error",
    "-i",
  ...
  ytdlp.stderr.resume();
  ffmpeg.stderr.resume();
  (0, import_promises3.pipeline)(ytdlp.stdout, ffmpeg.stdin).catch(() => {
  });
  return new Promise((resolve, reject) => {
    const chunks = [];
